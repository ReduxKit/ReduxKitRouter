...
func ReduxKitRouterMiddleware<State>(store: Store<State>) -> DispatchTransformer {
    return { next in
        { action in
            ...
            case let action as RouteChangeAction:
                ...
                do { try handleRouteChangeAction(store, action: action) }
                catch { print(error) }
            ...
            case let action as RouteBackAction:
                ...
                do { try handleRouteBackAction(store, action: action) }
                catch { print(error) }
 ...
 */
func handleRouteBackAction<State>(store: Store<State>, action: RouteBackAction) throws {
 ...
 */
func handleRouteChangeAction<State>(store: Store<State>, action: RouteChangeAction) throws {
